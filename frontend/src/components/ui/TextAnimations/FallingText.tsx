...
const FallingText: React.FC<FallingTextProps> = ({
  ...
  useEffect(() => {
    ...
    const newHTML = words
      .map((word) => {
        const clean = (str: string) => str.replace(/[.,]/g, '').toLowerCase();
        const isHighlighted = highlightWords.some((hw) => clean(word) === clean(hw));
        return `<span
  ...
  useEffect(() => {
    ...
    console.log('Initialisation FallingText...');
    ...
    const { Engine, Render, World, Bodies, Runner, Mouse, MouseConstraint } =
      ...
      if (!containerRef.current || !canvasContainerRef.current) {
        console.log('❌ Références manquantes');
        return;
      }
    ...
    console.log('✅ Animation FallingText démarrée');
    ...
    return () => {
      ...
      console.error('Erreur dans FallingText:', error);
    }
  }, [
    effectStarted,
    gravity,
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          fontSize: window.innerWidth < 768 ? '1.5rem' : fontSize,
          lineHeight: 1.4,
        }}
      />
